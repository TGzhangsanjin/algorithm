 ...
 *   2. 节点a往上找到的头结点，叫做 a 所在集合的代表节点
 *   3. 查询a和b是不是在同一个集合，就是看找到的代表节点是不是同一个
 *   4. 把a和b各自所代表的的集合合并成一个集合，只需要将小集合的代表点挂在大集合代表点下方即可
 * 路径压缩：
 *  1. 节点往上找代表点的过程，把沿途的链都变成扁平的（路径压缩）
 *  2. 小集合挂在大集合下面， 会让链的层级增长的更慢
 *  3. 如果方法调用很频繁，那么单次调用的代价是 O(1), 两个方法都是如此
 *
 *  常用应用：
 *      1. 解决两大块区域合并的问题
 *      2. 常用在图等领域中
